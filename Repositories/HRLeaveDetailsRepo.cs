...
namespace RMS.Repositories
{
    public class HRLeaveDetailsRepo : IHRLeaveDetails
    {
        ...
        public HRLeaveDetailsRepo(RmsDbContext context) // will be passed by dependency injection.
        {
            _context = context;
        }
        ...
        public bool IsNameExists(int Did, int id)
        {

            int ct = _context.HRLeaveDetail.Where(n => n.HREDId == Did && n.HRLDId != id).Count();
            if (ct > 0)
                return true;
            else
                return false;
